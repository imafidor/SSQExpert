  ...
  onCheckChanged = (e) => {
    ...
    if (e.target.checked) {
      this.setState((prevState) => ({
        selectedLabs: [...prevState.selectedLabs, name],
      }));
    } else {
      this.setState((prevState) => ({
        selectedLabs: prevState.selectedLabs.filter((lab) => lab !== name),
      }));
      // this.state.selectedLabs.filter((lab) => lab !== e.target.name)
    }
  };
  moveToNextStep = () => {
    this.props.closeLabs(this.state.selectedLabs);
    this.props.nextStep();
  };
      ...
      return (
        <motion.div
          ...
          animate={{
            x: 0,
            opacity: 1,
            transition: { delay: `${m}`, duration: 0.3 },
          }}
        ...
        >
          <input
            id={`option${x}`}
            name={lab}
            checked={this.state.selectedLabs.includes(lab)}
            className="input"
            type="checkbox"
            onChange={this.onCheckChanged}
          />
          <label for={`option${x}`} className="label">
            {lab}
          </label>
        </motion.div>
      );
        ...
        {this.props.open && (
          ...
          >
            ...
            >
              ...
              >
                Please select the laboratories available for the programme:
              </motion.h3>
              {labs}
              <Button
                sx={{ mt: 5 }}
