...
class StaffOffices extends PureComponent {
    ...
    addRow=(tableHeaderList)=>(e)=>{
        ...
        var rowData = tableHeaderList.reduce((result,item)=>{
            if(item==='id'){ 
                result[item]= this.state.officeData[this.state.officeData.length - 1][item] + 1;
              return result;
            }
            else{ 
              result[item] = "";
          ...
          this.setState({officeData:officeDataClone},()=>{
            ...
            console.log(dataIndex);
                ...
                return (<td key = {data}><input onChange = {this.handleChange(rowData['id'] - 1, data)} row = {rowData['id'] - 1}  column={data}  type={((data==='Capacity (No of Lecturers)')||(data==='Size in (MeterSquare)'))?"number":"text"} /></td>);
                }
                }
                )
...
componentDidMount(){
    this.initializeTable(this.table14HeaderList);
         ...
         render() {
        return (<AnimatePresence><div className='container4' initial={{scale:0}} animate ={{scale:1,transition:{duration:0.3}}} exit={{scale:0,transition:{delay:0.6}}}>
 <motion.h3 initial ={{x:500,opacity:0}} animate={{x:0,opacity:1, transition:{delay:0.3,duration:0.3}}} exit={{x:-500,opacity:0}}>   Office Accommodation</motion.h3>
  <motion.p initial ={{x:500,opacity:0}} animate={{x:0,opacity:1, transition:{delay:0.6,duration:0.3}}} exit={{x:-500,opacity:0,transition:{delay:0.3}}} >Provide the following information on offices (including Departmental office)
  ...
  <TableControls>
  <button style={{color:'#944317'}} onClick={this.deleteLastRow} >DELETE LAST ROW</button>
     <button style={{color:'#5C9210'}} onClick= {this.addRow(this.table14HeaderList)} >ADD ROW</button> 
  </TableControls>
