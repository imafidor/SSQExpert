...
class Laboratories extends Component {
    constructor(props) {
        super(props)
        this.workingConditionOptions=["Functional","Non Functional","Installed", "Not Installed"]
        this.table6and7HeaderList=['id','Item Description and Model','Quantity','Working Condition']
        this.state = {
        unchanged:true,
        laboratoryRows:[],
        laboratoriesData:[],
   ...
   laboratoryDataClone[index].pop();
    ...
    addRow=(tableHeaderList,laboratory,index)=>(e)=>{
        ...
        var rowData = tableHeaderList.reduce((result,item)=>{
            if(item==='id'){ 
                ...
                result[item]= this.state.laboratoriesData[index][this.state.laboratoriesData[index].length - 1][item] + 1;
              return result;
            }
            else{ 
              result[item] = "";
              ...
              //  native= {true}
               >
                ...
                {labItems} 
                  ...
                  return(<td key={data}> <FormControl variant="filled" sx={{ m: 1, minWidth: 120 }}>
                  <InputLabel id="demo-simple-select-filled-label">Working Condition</InputLabel>
                  <Select
                    labelId="demo-simple-select-filled-label"
                    id="demo-simple-select-filled"
                    //  value={this.state.laboratoriesData[index][rowData['id'] - 1][data]}
                  ...
                  return (<td key = {data}><input type="number"  onChange = {this.handleChange(rowData['id'] - 1, data,index)}  row = {rowData['id'] - 1} column = {data}   /></td>);
                }
          }
              )
    ...
    initializeTable=(tableHeaderList,laboratory,index)=>{
        ...
        var labItems=this.state.labWithEquipments[laboratory].map((labItem,i)=>{
            return <MenuItem key = {Math.random} value={labItem}>{labItem}</MenuItem>
        });
        
        var workingConditionOptions= this.workingConditionOptions.map((item,j)=>{
        ...
        var rowData = tableHeaderList.reduce(function(result,item){
          ...
          this.setState(prevState => ({
            ...
            //  value={this.state.laboratoriesData[index][rowData['id'] - 1][data]}
                 ...
                 return (<td key = {data}><input type="number"   row = {rowData['id'] - 1} column = {data}     onChange={this.handleChange(rowData['id'] - 1, data,index)}/></td>);
               }
               }
               )
               var wrappedTableRow= <tr key = {rowData['id']}>{tableRow}</tr>;
...
var tables= this.state.laboratories.map((lab,index)=>{
 ...
 <Table>
    <tbody key = {index}>
     <tr  key = {index}>{this.renderTableHeaderList(this.table6and7HeaderList)}</tr>
        {this.state.laboratoryRows[index]}    
    </tbody>
</Table>

<TableControls>
     <button style={{color:'#944317'}} onClick={this.deleteLastRow(index)}>DELETE LAST ROW</button>
     <button style={{color:'#5C9210'}} onClick= {this.addRow(this.table6and7HeaderList,lab,index)} >ADD ROW</button>
</TableControls>
